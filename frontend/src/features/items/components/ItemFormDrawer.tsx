  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Input placeholder="e.g. Brown Jeera Rice" size="large" />
        </Form.Item>
        ...
        <Form.Item
          ...
          rules={[
            ...
            { type: 'number', min: 0, message: 'Base price cannot be negative' },
          ]}
        >
          <InputNumber
            size="large"
                  ...
                  >
                    <Form.Item
                      ...
                      rules={[
                        {
                          validator: (_, value) => {
                            const valueField = form.getFieldValue([
                              'variants',
                              index,
                              'value',
                            ]);
                            if (!valueField && !value) return Promise.resolve();
                            if (value) return Promise.resolve();
                            return Promise.reject(
                              new Error('Variant name is required when value is set'),
                            );
                          },
                        },
                      ]}
                    ...
                    <Form.Item
                      ...
                      rules={[
                        {
                          validator: (_, value) => {
                            const nameField = form.getFieldValue([
                              'variants',
                              index,
                              'name',
                            ]);
                            if (!nameField && !value) return Promise.resolve();
                            if (value) return Promise.resolve();
                            return Promise.reject(
                              new Error('Variant value is required when name is set'),
                            );
                          },
                        },
                      ]}
